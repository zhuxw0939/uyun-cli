<template>
	<div class="_center">
		<top curtitle="学校管理" :other="false" :editwin="editwin"></top>
		<div class="table">
			<el-table :data="tableData" border style="width: 100%" :max-height="maxHeight">
				<el-table-column prop="name" label="学校分类" width="150"></el-table-column>
				<el-table-column prop="phaseName" label="学校类别" width="150"></el-table-column>
		...
		<countrywin curmodel="school" :editwin="editwin" @loadlist="listData"></countrywin>
	</div>
</template>
    ...
    export default{
		...
		methods: {
			listData(){
				var obj = this;
				// 查询当前区县学校分类列表
				this.$http.post("/api/servers/ExamSchoolGroupApi_page", {
					body: {
					...
					data = data.body;
					console.info("学校管理列表："+JSON.stringify(data));
					if(data.code == 200 && data.success == true){
						...
						this.$message.error('学校管理列表接口返回失败!');
					}
				}, function(error){
					console.info("返回失败"+error);
					this.$message.error('学校管理列表接口错误!');
				});
			},
			...
			deleteRow(index, rows) {
				console.log("删除学校分类："+rows[index].id);
				if(rows[index].id){
					this.$confirm('此操作将删除该数据, 是否继续?', '提示', {
						...
						// 删除乡镇
						this.$http.post("/api/servers/deleteSchool", {
							id: rows[index].id
						}).then(function(data){
							data = data.body;
							console.info(data);
							if(data.code == 200 && data.success == true){
								...
								this.$message.error('删除学校分类接口返回失败!');
							}
						}, function(error){
							console.info("返回失败："+error);
							this.$message.error('删除学校分类接口错误!');
						});
					});
				}else {
